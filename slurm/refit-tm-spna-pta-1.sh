...
#SBATCH --job-name refit-tm-spna-pta-1
#SBATCH --mail-user=wrightd2@oregonstate.edu
#SBATCH --mail-type=ALL
#SBATCH --output ./logs/refit-tm-spna-pta-1-%A-%a-stdout.log
#SBATCH --error ./logs/refit-tm-spna-pta-1-%A-%a-stderr.log
...
$PIXI run mpirun --mca pml ob1 -n $SLURM_NTASKS  python ../simulation.py update-timing-models \
    --fake-psrs "$DATADIR/fake_feathers_pta_1/fake_psrs.pkl" \
    --map-params "$DATADIR/fake_feathers_pta_1/spna-max-likelihood-estimate.json" \
    --outdir "$DATADIR" \
    --pta-suffix "pta_1" \
    --use-mpi
